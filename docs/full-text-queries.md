...
**Note**, that even though some methods are explained in [match query](#match) and 
[match phrase query](#match-phrase), they can also be used in match phrase prefix query. For example,
...
**Note**, that some methods are explained in [match query](#match), but they can also be used in 
match phrase query. For example, you can use `analyzer` as follows:
...
**Note**, that even though some methods are explained in [match query](#match) and 
[match phrase query](#match-phrase), they can also be used in multi-match query. 
